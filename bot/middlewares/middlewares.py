...
class CallbackAgeMiddleware(BaseMiddleware):
    async def __call__(self, handler, event, data):
        if isinstance(event, CallbackQuery):
            request_time = datetime.fromtimestamp(event.id / 4294967296, tz=pytz.timezone(app_settings.timezone))
            if (datetime.now(pytz.timezone(app_settings.timezone)) - request_time).total_seconds() > 30:
                logger.warning(f"Устаревший callback от {event.from_user.id}, игнорирую")
                return None
        return await handler(event, data)
